...
import { IconPlus, IconArrowRight, IconChevronUpDown } from "./core/icons";
import { models, type Model, types } from "../constants/models";
              ...
              <CommandList className="h-[var(--cmdk-list-height)] max-h-[400px]">
                ...
                {types.map((type) => (
                  <CommandGroup key={type} heading={type}>
                    {models
                      .filter((mdl) => mdl.type === type)
                      .map((mdl) => (
                        <ModelItem
                          key={mdl.id}
                          model={mdl}
                          isSelected={mdl.id === model.id}
